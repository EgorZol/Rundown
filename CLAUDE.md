...
- Вызывать ТОЛЬКО в "Утро" и "Спорт" хендлерах. Семафор `self._global_sync_sem` (лимит = 1) оставлен в `bot.py` для защиты от банов (HTTP 429) со стороны Garmin при одновременных запросах.
