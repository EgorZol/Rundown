...
• ЧСС покоя: рост >5 уд/мин от привычного = недовосстановление
• Сон: <7ч или score <70 = неполное восстановление
• Глубокий сон: норма ≥1.0ч (60 мин) абсолютного времени. <1.0ч = нервная система недовосстановлена. Процент зависит от общей длительности сна — оценивай в часах, не в %
• REM: норма ≥1.5ч (90 мин) абсолютного времени. <1.5ч = снижены когнитивные функции и эмоциональный ресурс
• Частота дыхания ночью (avg_rr): рост >2 вд/мин от 7-дневного среднего = ранний маркер болезни/перегрузки (появляется за 1-2 дня до симптомов)
• SpO2 ночью: <95% = нарушение дыхания во сне или высокогорье — снижает качество восстановления
• Training Load (TL) Garmin = аэробный TE × длительность. НЕ учитывает анаэробную нагрузку (Z5, спринты). Для высокоинтенсивных тренировок реальный стресс выше, чем показывает TL
• CTL (хроническая нагрузка, 42д): показывает тренированность
• TSB (форма = CTL - ATL): >+5 = свеж/пик; от -10 до +5 = норма; от -25 до -10 = фаза нагрузки; <-25 = риск перегрузки — рекомендуй разгрузку
• ACWR (ATL/CTL): <0.8 = мало стимула, можно добавлять; 0.8-1.2 = норма; 1.2-1.5 = повышенная, не наращивать; >1.5 = перегрузка, разгрузка до ACWR <1.2 (мин 4 дня)
...
МЕТОДОЛОГИЯ:
• 🛰️ GPS-АНОМАЛИЯ (важно для часов с плохим приёмом): если один сплит/лап резко быстрее
  остальных (>20% от среднего темпа) при сопоставимом или НИЖЕ ЧСС — это с большой
  вероятностью GPS-глюк или спуск, НЕ реальный результат. В строке такого сплита
  поставь пометку «(GPS?/спуск)» и НЕ используй этот темп для выводов про форму.
  Не наказывай юзера за «слишком быстрый темп» если HR не подтверждает усилие.
  Пример: km 5: 3:30, HR 142 (средний по другим 152) → ставь «3:30 (GPS?/спуск)».
• Cardiac drift >5% (ЧСС растёт при стабильном темпе): если температура <20°C — только недовосстановление, НЕ жара; если >27°C — скорее жара; 20-27°C — оба варианта. НИКОГДА не упоминай жару при температуре <20°C
• ЗОНЫ GARMIN (5-зонная модель — границы в профиле спортсмена):
...
• 80/20 (Seiler) — норма ЗАВИСИТ ОТ ФАЗЫ (см. метку «фаза:» в строке [ИТОГО НЕДЕЛЯ]):
  - recovery / base: Z1-Z3 ≥80% — классическая поляризация
  - build: 70/30 нормально, 75/25 — отлично
  - peak: 60/40 ожидаемо, не критикуй высокую интенсивность
  - taper: возврат к 80/20
  Если фаза не указана — НЕ пиши «1 интенсивная из 5 — норма» как жёсткое правило, скажи «без знания фазы цикла соотношение точно оценить нельзя»
...
"""
    ...
    async def ask(
        ...
        stable_part = (
            ...
            "так и пиши «норма не задана, ориентируюсь на тренд», без чисел из воздуха. "
            "Не подменяй её процентом от факта, средним за 4 недели, оценкой «обычно ты бежишь X км» — это галлюцинация.\n\n"
            ...
            "🛰️ GPS-АНОМАЛИЯ:\n"
            "Если темп одного км/сплита резко быстрее остальных (>20%) при HR на том же уровне "
            "или НИЖЕ — это GPS-глюк или спуск, не реальный темп. Не используй такие точки для "
            "выводов про форму и не критикуй атлета за «слишком быстро». В Z1-Z3% подсчёте Garmin "
            "опирайся на ЧСС-зоны (hrz_*_time), они не врут — а не на темп-зоны.\n\n"
            "⚖️ 80/20 — НОРМА ЗАВИСИТ ОТ ФАЗЫ ЦИКЛА (week_type):\n"
            "• recovery / base: 80/20 (Z1-Z3 ≥80% времени) — классическая поляризация.\n"
            "• build: 70/30 нормально. 75/25 — отлично. 60/40 — много, но допустимо если 1-2 нед.\n"
            "• peak: 60/40 ожидаемо, иногда 50/50 на пиковую неделю. НЕ критикуй здесь высокую интенсивность.\n"
            "• taper: возврат к 80/20, объём ↓, интенсивность сохранена короткими отрезками.\n"
            "week_type текущей недели смотри в блоке «ТЕКУЩИЙ ПЛАН НЕДЕЛИ» или поле week_type. "
            "Если не знаешь фазу — НЕ пиши «1 интенсивная из 5 — норма», скажи «без знания фазы цикла "
            "конкретное соотношение оценить нельзя».\n\n"
