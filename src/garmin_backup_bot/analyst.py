...
class HealthAnalyst:
    ...
    async def analyze_workout(self, activities: list[dict[str, Any]], daily_metrics: dict[str, Any] | None, history: list[dict] | None = None, user_memory: str = "", plan_text: str = "") -> str:
...
• 80/20 (Seiler): 80% СЕССИЙ в Z1-Z3, 20% интенсивных (Z4-Z5). Зоны — данные Garmin, без пересчёта
...
"""
    ...
    async def analyze_weekly_summary(
        ...
        parts = [f"=== ИТОГ НЕДЕЛИ {week_start.strftime('%d.%m')}–{today.strftime('%d.%m')} ===\n"]

        activities = metrics.get("activities_28d") or []
        week_acts = [
            a for a in activities
            if a.get("start_time", "") >= week_start.isoformat()
        ]
...
"""
    ...
    async def ask(
        ...
        stable_part = (
            ...
            "[ЗАПОМНИТЬ: краткая формулировка]\n"
            "Примеры:\n"
            "• «у меня болит ахилл» → [ЗАПОМНИТЬ: болит ахилл — исключить прыжковые и горки]\n"
