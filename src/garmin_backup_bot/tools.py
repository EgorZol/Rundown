...
def build_tool_schemas(
    ...
    tools = [
        {
            ...
            "description": (
                ...
                "• weight — day, weight\n"
                "• body_composition — day, weight, bmi, fat_pct, muscle_kg, muscle_pct, "
                "water_pct, bone_kg, protein_pct, visceral_fat, bmr_kcal, body_age "
                "(умные весы; есть НЕ у всех атлетов — при отсутствии таблицы просто "
                "скажи, что весы не подключены. fat_pct=0 — брак измерения, отфильтруй)\n"
                "Таблиц stress (внутридневной ряд) и sleep_events НЕТ — стресс бери из "
                "sleep.avg_stress (ночь) и daily_summary.stress_avg (день).\n"
        ...
        },
        {
            ...
            "description": (
                ...
                "Таблица activity_records (посекундные точки) НЕ ЗАПОЛНЯЕТСЯ после миграции "
                "синка — не используй её; темп/пульс по кускам бери из activity_laps.\n"
    ...
    if write_tools and "set_plan_preferences" in write_tools:
        tools.append({
            "name": "set_plan_preferences",
            "description": (
                "Сохрани ПОСТОЯННЫЕ пожелания атлета к недельным планам — конвейер "
                "планов учитывает их при каждой генерации. Сюда: дни интенсивных, "
                "день лонга, целевой недельный объём, минимальная разминка, любые "
                "устойчивые предпочтения по структуре недели. Передавай ПОЛНЫЙ новый "
                "текст (замещает прежний — объедини старые пожелания из контекста с "
                "новыми; пустая строка = очистить). Вызывай, когда атлет высказывает "
                "пожелание о БУДУЩИХ планах; после сохранения предложи перестроить "
                "план. Разовые правки текущей недели сюда НЕ пиши."
            ),
            "input_schema": {
                "type": "object",
                "properties": {"text": {"type": "string",
                    "description": "полный текст пожеланий, например: «интенсивные вт и чт; лонг вс; объём ~60 км; разминка минимум 3 км»"}},
                "required": ["text"],
            },
        })
