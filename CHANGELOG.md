...
Большое многоагентное ревью (6 осей, 43 находки, адверсариальная проверка
каждой скептиком: 40 подтверждено, 3 отклонено) и исправление ВСЕХ
подтверждённых находок за четыре батча.

### Fixed — ядро тренера
- **Пороги recovery ожили**: код читал ключи, которых пайплайн метрик не
  производит (total_sleep_secs, resting_hr.last, body_battery.min,
  fitness.acwr) — RHR-спайк, короткий сон, низкий BB и рост дыхания НЕ
  срабатывали никогда; вердикты импровизировала модель. Новый
  coach._morning_inputs нормализует реальные формы (строки времени сна,
  медиана rhr_trend_7d, BB из daily_summary, ACWR=ATL/CTL).
- Приоритет вердиктов только ухудшается (_worsen: alarm не понижается);
  HRV LOW с ночью вне базы → poor; «ноль сна» = 0.0ч; самочувствие ≤2
  требует смежных дат; границы TSB согласованы.
- **Бег = RUN_SPORTS везде** (20 мест): дорожка/трейл/манеж/улица больше
  не выпадают из километража, 80/20, plan_adherence, темпов и подсказки
  SQL-тулу (у одного юзера мимо статистики шли 69 из 167 пробежек).
- Regex дат плана не бэктрекается на «DD.MM.YYYY» (порча текста плана).

### Fixed — данные и синк
- stop_time активности = start + elapsed (был копией старта у 100% строк).
- Km-сплиты наконец читаются (splits/{id}.json писался в пустоту, читатели
  смотрели в мёртвую activity_records); круги — в локальном времени, где
  API его отдаёт.
- daily_summary не затирается NULL при фейле Garmin API; юзер без данных
  сна не застревает вечно в «первичном синке».
- Описания SQL-тулов очищены от несуществующих таблиц (stress,
  sleep_events, activity_records) + маркер «ОБРЕЗАНО» при 200+ строках.

### Fixed — деньги (до включения ЮKassa)
- Идемпотентность successful_payment по telegram_charge_id.
- «30 дней» = ровно 30, триал 7 = 7 (paid_until включительно).
- Смена тарифа конвертирует неиспользованный остаток по деньгам
  (например, 10 дн Тренера → 49 дн Калорий) с пометкой юзеру.
- Коллбеки еды под пейволлом; rate-limit веб-формы не обходится
  подделкой X-Forwarded-For, словарь IP не растёт бесконечно.

### Fixed — флоу и честность LLM
- Фолбэк модели запрещён после исполнения write-tool — двойные записи
  фактов/гонок исключены; при лимите tool-итераций ответ честно
  предупреждает о невыполненном.
- Двойной тап Утро/Разбор/План не даёт двойной синк (атомарный busy-флаг).
- Серия фото еды не теряет приёмы (автосохранение вытесняемого
  распознавания); голосовая правка еды — коррекция, а не новое блюдо;
  из анкеты профиля можно выйти («стоп»/«отмена»).
- QA: вопрос пишется в историю до tool-цикла (хронология); QA-контекст
  производит план-маркеры; cardiac drift и 80/20 имеют по одному
  источнику истины; промпт-рассинхроны выправлены (ПИТАНИЕ вчера,
  [ИТОГО НЕДЕЛЯ] в Пн-Вт, описание «🔥 Калории»).
